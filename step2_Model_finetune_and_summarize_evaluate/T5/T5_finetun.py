...
# ==========================================================
# Only update the fixed paths here
# ==========================================================

# Local base T5 checkpoint
REPO_ROOT = Path(__file__).resolve().parents[2]
PRIVATE_ROOT = REPO_ROOT / "local_private_data"
MODEL_CHECKPOINT = PRIVATE_ROOT / "models" / "T5-base"

# Train, validation, and test JSON files
...
# Output directory
OUT_DIR = PRIVATE_ROOT / "step2" / "generated" / "training" / "T5"
FINAL_HF_DIR = PRIVATE_ROOT / "models" / "T5_final"
...
# ==========================================================
# Fixed hyperparameters aligned with the RadSumBART setup
# ==========================================================
SEED = 5
LR = 2e-5
EPOCHS = 20
...
def eval_rouge(dataloader, model, tokenizer, device) -> Dict[str, float]:
    ...
    for i, (hyp, ref) in enumerate(zip(preds, refs)):
        hyp = str(hyp or "").strip()
        ref = str(ref or "").strip()

        try:
            score = rouge.get_scores([hyp], [ref], avg=True)
            total_r1 += float(score["rouge-1"]["f"])
            total_r2 += float(score["rouge-2"]["f"])
            total_rl += float(score["rouge-l"]["f"])

        except ValueError as e:
            # Treat empty / invalid hypothesis as zero-score case.
            # This keeps the sample in validation instead of dropping it.
            if "Hypothesis is empty" in str(e) or "Reference is empty" in str(e):
                invalid_count += 1
                ...
                # Add 0.0 to all ROUGE totals by doing nothing.
                continue
            else:
                raise
    ...
    if invalid_count > 0:
        print(f"[WARN] ROUGE invalid cases counted as zero: {invalid_count}/{n}")
        print(f"[WARN] First invalid examples: {invalid_examples}")
