# -*- coding: utf-8 -*-
# Fine_Tune_train_fix2.py
# One-click fine-tune for summarization (NO CLI args)
# Robust JSON loader: supports list[dict], dict[id]=dict, {"data":[...]} etc.
# Exports final HF model dir: RadSumBART_final/
...
MODEL_CHECKPOINT = PRIVATE_ROOT / "models" / "RadSumBART_pretrained" / "checkpoint-617925"
...
# Path and file settings
OUT_DIR = PRIVATE_ROOT / "step2" / "generated" / "training" / "RadSumBART"
FINAL_HF_DIR = PRIVATE_ROOT / "models" / "RadSumBART_final"
...
MAX_DATASET_SIZE = 10**9  # Dataset size cap
...
def load_samples(json_path: Path, max_n: int) -> List[Dict[str, str]]:
    """
    Load samples from JSON file robustly and normalize into:
...
def build_collate_fn(tokenizer, model):
    def collate_fn(batch: List[Dict[str, str]]) -> Dict[str, torch.Tensor]:
        ...
        if hasattr(model, "prepare_decoder_input_ids_from_labels"):
            enc["decoder_input_ids"] = model.prepare_decoder_input_ids_from_labels(labels)

        # mask tokens after eos
        eos = tokenizer.eos_token_id
        if eos is not None:
            end_token_index = torch.where(labels == eos)[1]
            for i, end_idx in enumerate(end_token_index):
                if end_idx + 1 < labels.size(1):
...
def eval_rouge(dataloader, model, tokenizer, device) -> Dict[str, float]:
    ...
    for batch in dataloader:
        ...
        # decode labels (replace -100 to pad for decode)
...
def main():
    ...
    if len(train_samples) == 0 or len(val_samples) == 0:
        raise RuntimeError(
            ...
            "Please open one JSON and confirm it contains dicts with findings/impression fields."
        )
    ...
    # -------- load model/tokenizer from MLM checkpoint --------
    ...
    collate_fn = build_collate_fn(tokenizer, model)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(f"[VALID] epoch={epoch} ROUGE_avg={rouge_avg:.4f} (R1={valid_scores['rouge1_f']:.4f}, R2={valid_scores['rouge2_f']:.4f}, RL={valid_scores['rougeL_f']:.4f})")
    ...
    meta = {
        "base_mlm_checkpoint": str(MODEL_CHECKPOINT),
